...
Build docker images.   Use -h for help."""
    ...
    existing_plugins = []
    with open('files/plugins.txt', 'r+') as f:
    ...
    if len(old_plugins) == len(new_plugins) and len(old_plugins) > 0:
        for old_plugin, new_plugin in zip(old_plugins, new_plugins):
            ...
            lines.sort()
            data = '\n'.join(lines) + '\n'
            with open('files/plugins.txt', 'wt') as f:
                f.write(data)
            ...
            subprocess.check_call(['git', 'commit', '-m', message, 'files/plugins.txt', os.path.join('ref', 'plugins')])
